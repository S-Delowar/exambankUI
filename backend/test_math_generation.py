#!/usr/bin/env python3
"""Test math solution generation with a small batch."""
import asyncio
import logging
...
BATCH_SIZE = 5  # Test with 5 questions
...
async def test_batch():
    ...
    if not rows:
        logger.info("No pending mathematics questions found.")
        return

    logger.info(f"Testing with {len(rows)} mathematics questions")
    ...
    for idx, q in enumerate(rows, 1):
        try:
            ...
            logger.info(f"[{idx}/{len(rows)}] ✓ {q.exam_session} Q{q.question_number}: Book={q.correct_answer}, Gemini={result.label}")
