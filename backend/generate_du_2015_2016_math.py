#!/usr/bin/env python3
"""Generate math solutions for DU 2015-2016 session."""
import asyncio
import logging
...
async def generate_du_2015_2016():
    ...
    async with SessionLocal() as session:
        stmt = (
            ...
            .where(AdmissionMcqQuestion.exam_session == "2020-2021")
            .order_by(AdmissionMcqQuestion.question_number)
        )
        rows = list((await session.execute(stmt)).scalars().all())

    if not rows:
        logger.info("No DU 2020-2021 math questions found.")
        return

    logger.info(f"Found {len(rows)} DU 2020-2021 math questions\n")
    ...
    for idx, q in enumerate(rows, 1):
        try:
            ...
            logger.info(f"[{idx}/{len(rows)}] ✓ Q{q.question_number}: Book={q.correct_answer}, Gemini={result.label}")
            logger.info(f"  Solution: {result.solution[:100]}...\n")
            await asyncio.sleep(4.0)

        except Exception as e:
            logger.error(f"[{idx}/{len(rows)}] ✗ Q{q.question_number}: {e}\n")
            await asyncio.sleep(4.0)


if __name__ == "__main__":
    asyncio.run(generate_du_2015_2016())
