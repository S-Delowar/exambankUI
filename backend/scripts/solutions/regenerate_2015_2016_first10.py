...
async def regenerate():
    ...
    async with SessionLocal() as session:
        stmt = (
            ...
            .where(AdmissionMcqQuestion.exam_session == "2015-2016")
            .order_by(AdmissionMcqQuestion.question_number)
            .limit(10)
        )
        rows = list((await session.execute(stmt)).scalars().all())

    logger.info(f"Found {len(rows)} math questions for 2015-2016 (first 10)")
    ...
    for idx, q in enumerate(rows, 1):
        try:
            ...
            logger.info(f"[{idx}/{len(rows)}] ✓ Q{q.question_number}: Ans={result.label}")
