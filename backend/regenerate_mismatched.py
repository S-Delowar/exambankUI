...
async def regenerate():
    ...
    async with SessionLocal() as session:
        stmt = (
            ...
            .where(AdmissionMcqQuestion.correct_answer != AdmissionMcqQuestion.gemini_correct_answer)
            .order_by(AdmissionMcqQuestion.exam_session, AdmissionMcqQuestion.question_number)
        )
        rows = list((await session.execute(stmt)).scalars().all())

    logger.info(f"Found {len(rows)} mismatched questions")
    ...
    for idx, q in enumerate(rows, 1):
        try:
            ...
            logger.info(f"[{idx}/{len(rows)}] ✓ {q.exam_session} Q{q.question_number}: {q.correct_answer}→{result.label}")
